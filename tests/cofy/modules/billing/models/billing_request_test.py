import datetime as dt
from unittest.mock import MagicMock
from zoneinfo import ZoneInfo

import pandas as pd
import pytest
from energy_cost import Contract, Meter, MeterType, PowerDirection, Supplier, Tariff
from energy_cost.data import ConnectionType, CustomerType, RegionalData
from pydantic import ValidationError

from cofy.modules.billing.models.billing_request import (
    BillingRequest,
    ContractInfo,
    DataPoint,
    MeterInfo,
    _Ref,
)


class TestDataPoint:
    def test_creates_with_valid_fields(self):
        ts = dt.datetime(2024, 1, 1, tzinfo=dt.UTC)
        dp = DataPoint(timestamp=ts, value=150.5)
        assert dp.timestamp == ts
        assert dp.value == 150.5


class TestMeterInfo:
    def _dp(self, value: float = 100.0, day: int = 1) -> DataPoint:
        return DataPoint(timestamp=dt.datetime(2024, 1, day, tzinfo=dt.UTC), value=value)

    def _two_dps(self) -> list[DataPoint]:
        return [self._dp(100.0, day=1), self._dp(200.0, day=2)]

    def test_to_meter_has_default_direction_and_type(self):
        info = MeterInfo(data=self._two_dps())
        meter = info.to_meter()
        assert isinstance(meter, Meter)
        assert meter.direction == PowerDirection.CONSUMPTION
        assert meter.type == MeterType.SINGLE_RATE

    def test_to_meter_preserves_explicit_direction_and_type(self):
        info = MeterInfo(direction=PowerDirection.INJECTION, type=MeterType.TOU_PEAK, data=self._two_dps())
        meter = info.to_meter()
        assert meter.direction == PowerDirection.INJECTION
        assert meter.type == MeterType.TOU_PEAK

    def test_to_meter_data_matches_datapoints(self):
        dps = [
            DataPoint(timestamp=dt.datetime(2024, 1, 1, tzinfo=dt.UTC), value=10.0),
            DataPoint(timestamp=dt.datetime(2024, 1, 2, tzinfo=dt.UTC), value=20.0),
        ]
        info = MeterInfo(data=dps)
        meter = info.to_meter()
        assert isinstance(meter.data, pd.DataFrame)
        assert list(meter.data["value"]) == [10.0 / 1000, 20.0 / 1000]
        assert list(meter.data["timestamp"]) == [dp.timestamp for dp in dps]

    def test_raises_for_empty_data(self):
        with pytest.raises(ValidationError):
            MeterInfo(data=[])

    def test_raises_for_single_datapoint(self):
        with pytest.raises(ValidationError):
            MeterInfo(data=[self._dp()])


class TestContractInfo:
    def setup_method(self):
        self.mock_tariff = MagicMock(spec=Tariff)
        Supplier.register("mock_supplier", Supplier(products={"prod1": self.mock_tariff}))
        RegionalData.register(
            ("mock_region", ConnectionType.ELECTRICITY),
            RegionalData(
                distributors={"dist1": self.mock_tariff},
                fees={ct: MagicMock() for ct in CustomerType},
                taxes=[],
                timezone=ZoneInfo("UTC"),
            ),
        )
        dps = [
            DataPoint(timestamp=dt.datetime(2024, 1, 1, tzinfo=dt.UTC), value=1.0),
            DataPoint(timestamp=dt.datetime(2024, 1, 2, tzinfo=dt.UTC), value=2.0),
        ]
        self.model = BillingRequest(
            meters=[MeterInfo(data=dps)],
            contract=ContractInfo(
                customer_type=CustomerType.RESIDENTIAL,
                product="prod1",
                distributor="dist1",
                supplier="mock_supplier",
                region="mock_region",
            ),
        )

    def test_to_contract_returns_contract_instance(self):
        contract = self.model.contract.to_contract()
        assert isinstance(contract, Contract)

    def test_to_contract_uses_selected_product(self):
        contract = self.model.contract.to_contract()
        assert contract.supplier is self.mock_tariff

    def test_to_contract_uses_selected_distributor(self):
        contract = self.model.contract.to_contract()
        assert contract.distributor is self.mock_tariff

    def test_to_contract_without_product_sets_none_supplier(self):
        dps = [
            DataPoint(timestamp=dt.datetime(2024, 1, 1, tzinfo=dt.UTC), value=1.0),
            DataPoint(timestamp=dt.datetime(2024, 1, 2, tzinfo=dt.UTC), value=2.0),
        ]
        model = BillingRequest(
            meters=[MeterInfo(data=dps)], contract=ContractInfo(customer_type=CustomerType.RESIDENTIAL)
        )
        contract = model.contract.to_contract()
        assert contract.supplier is None

    @pytest.mark.parametrize("customer_type", [ct.value for ct in CustomerType])
    def test_to_contract_for_all_customer_types(self, customer_type):
        dps = [
            DataPoint(timestamp=dt.datetime(2024, 1, 1, tzinfo=dt.UTC), value=1.0),
            DataPoint(timestamp=dt.datetime(2024, 1, 2, tzinfo=dt.UTC), value=2.0),
        ]
        model = BillingRequest(meters=[MeterInfo(data=dps)], contract=ContractInfo(customer_type=customer_type))
        contract = model.contract.to_contract()
        assert isinstance(contract, Contract)


class TestContractInfoObjectInput:
    """Verify that product/distributor can be provided as an object with an 'id' field."""

    def setup_method(self):
        self.mock_tariff = MagicMock(spec=Tariff)
        Supplier.register("mock_supplier", Supplier(products={"prod1": self.mock_tariff}))
        RegionalData.register(
            ("mock_region", ConnectionType.ELECTRICITY),
            RegionalData(
                distributors={"dist1": self.mock_tariff},
                fees={ct: MagicMock() for ct in CustomerType},
                taxes=[],
                timezone=ZoneInfo("UTC"),
            ),
        )

    def _two_dps(self) -> list[DataPoint]:
        return [
            DataPoint(timestamp=dt.datetime(2024, 1, 1, tzinfo=dt.UTC), value=1.0),
            DataPoint(timestamp=dt.datetime(2024, 1, 2, tzinfo=dt.UTC), value=2.0),
        ]

    def _meter(self) -> MeterInfo:
        return MeterInfo(data=self._two_dps())

    def test_product_as_id_only_object(self):
        model = BillingRequest(meters=[self._meter()], contract=ContractInfo(product=_Ref(id="prod1")))
        contract = model.contract.to_contract(default_region="mock_region", default_supplier="mock_supplier")
        assert isinstance(contract, Contract)

    def test_product_as_object_with_extra_fields(self):
        model = BillingRequest(meters=[self._meter()], contract=ContractInfo(product=_Ref(id="prod1")))
        contract = model.contract.to_contract(default_region="mock_region", default_supplier="mock_supplier")
        assert contract.supplier is self.mock_tariff

    def test_distributor_as_id_only_object(self):
        model = BillingRequest(meters=[self._meter()], contract=ContractInfo(distributor=_Ref(id="dist1")))
        contract = model.contract.to_contract(default_region="mock_region", default_supplier="mock_supplier")
        assert isinstance(contract, Contract)

    def test_distributor_as_object_with_extra_fields(self):
        model = BillingRequest(meters=[self._meter()], contract=ContractInfo(distributor=_Ref(id="dist1")))
        contract = model.contract.to_contract(default_region="mock_region", default_supplier="mock_supplier")
        assert contract.distributor is self.mock_tariff
